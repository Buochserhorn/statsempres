...
beta_CD = Coordinate_Descent(X,Y,iteration=1000, lambda=0.05,n=10000,p=10)
...
for (k in number_n){
  ...
  num_iter<-Coordinate_Descent1(X=X_new, Y=Y_new, iteration=2000, lambda=0.05, p=10, n=k)
  iteration_conv<- c(iteration_conv,num_iter)
}

## Display the result
matrix(cbind(number_n,iteration_conv),nrow=length(number_n),ncol=2,dimnames=list(c(1:length(number_n)),c("Number of n","Iteration")))
...
## Case 3: Vary the number of predictors p; Check how many iterations are needed for convergence
  ...
  for (p in number_p){
    ...
    num_iter<-Coordinate_Descent1(X=X_new, Y=Y_new, iteration=1000, lambda=0.05, p, n=10000)
    iteration_conv<- c(iteration_conv,num_iter)
  }
  
  matrix(cbind(number_p,iteration_conv),nrow=length(number_p),ncol=2,dimnames=list(c(1:length(number_p)),c("Number of p","Iteration")))
...
diabetes_X <- apply(diabetes[,'x'],2,function(X) return((X - mean(X)) / (sqrt(var(X) *(length(X)-1)))))
...
lar.fit <-
  lars(diabetes_X, diabetes_Y, type = "lar", intercept = TRUE,normalize = FALSE)
...
lasso.fit1<-
  glmnet(diabetes_X, diabetes_Y, intercept = TRUE, alpha = 1,
         standardize = FALSE,thres=1e-18, lambda = lambda_lar/nrow(diabetes_X))
