...
\includegraphics[width=\textwidth]{img/minmaxalgo2.png}
\caption{Figure 5.10 from \cite{DL15}}
\end{figure}
...
Def. 
$\Psi:\R^p\times\R^p\to\R$ \underline{majorizes} $f$ at $\beta\in\R^p$ if \[\forall\theta\in\R^p\quad \Psi(\beta,\theta)\ge f(\beta)\]
...
\item[(i) \& (iii)] Definiton of \underline{majorize}
...
\vspace{15pt}
Def. A function $f(x,y):\R^m\times\R^n\to\R$ is biconvex, if $f(x,y)$ is convex for each $x\in\R^m$ and $f(x,y)$ is convex for each $y\in\R^n$.
\end{frame}
\note{Mathematica: \texttt{3D plot (1-xy)\^{}2, x in [-2,2], y in [-2,2]}}
