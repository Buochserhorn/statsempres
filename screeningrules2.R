...
# From the book: "The first variable to enter the model has largest 
...
# smallest lambda with all zero coef is at lambda = 0.5659 
...
lasso.coef <- predict(lasso.mod, s = lam, 
                      type = "coefficients", newx = x) 

# calculating inner product 
absinprod2 <- apply(x, 2, function(k) 
  abs(sum((y - y.hat) * k)) / length(y))
