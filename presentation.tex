...
\vspace*{1.5mm}
        ...
        \(\sum_{i=1}^{N}x_{ij}r_{i}= \big \langle x_j,y\big\rangle -  \sum_{k}\big \langle x_j,x_k\big\rangle \beta_{\hat{k}}\) \\
\vspace*{3mm}      
...
\(+I(g_i=-1)\cdot \log(1-p(x_{i};\beta_0,\beta))\}-\lambda \lvert\lvert\beta\lvert\lVert_{1}  \) \\
    \vspace*{1mm}
...
\visible<2>{
\begin{figure}
\includegraphics[height=2cm]{img/dualascentstep.pdf}
\caption{Own illustration of the dual ascent step in the ADMM algorithm utilising dual decomposition based on Geoff Gordon and Ryan Tibshirani (2012), Uses of Duality, \url{https://www.cs.cmu.edu/~ggordon/10725-F12/slides/18-dual-uses.pdf.}}
\end{figure}
}
%}
...
\begin{frame}
\frametitle{ADMM for the Lasso - Problem}
...
\begin{frame}
\frametitle{ADMM for the Lasso - Update}
...
\caption{From Trevor Hastie, Robert Tibshirani, and Martin Wainwright (2015), Statistical learning with sparsity: the Lasso and generalizations, \emph{CRC Press;} Boca Raton, FL, 12(3), 45 -- 678.}
\end{figure}
\end{frame}
...
\vspace{30pt}
\only<2>{Def. A function $f(\alpha,\beta):\R^m\times\R^n\to\R$ is biconvex, if for each $\alpha\in\R^m$ the function $\alpha\mapsto f(\alpha,\beta)$ is convex  and for each $\beta\in\R^n$ the function $\beta\mapsto f(\alpha,\beta)$ is convex.
\vspace{5pt}
Analoguosly, a set $\mathcal{C}\subseteq\mathcal{A}\times\mathcal{B}$, for $\mathcal{A, B}$ convex sets, is called biconvex, if it is convex in both directions.}
\end{frame}
